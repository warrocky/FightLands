...
namespace FightLands
{
    class TerrainTile : LandObject
    {
        ...
        private Texture2D createTexture()
        {
            ...
            int noiseDownSampling = 4;
            Point noiseSamples = new Point(texture.Width, texture.Height);
            Point reducedNoiseSamples = new Point(texture.Width / noiseDownSampling, texture.Height / noiseDownSampling);
            float[,] landGreenessNoiseData = land.grassLandGreeness.getValues(reducedNoiseSamples, position - size/2f, size);
            float[,] noiseDirtData = land.dirtPatches.getValues(reducedNoiseSamples, position - size / 2f, size);
            
            float[,] noiseMountainChanceData = land.mountainChanceNoise.getValues(reducedNoiseSamples, position - size / 2f, size);
            //float[,] noiseMountainChanceData = new float[texture.Width, texture.Height];
            float[,] noiseMountainChainData = land.mountainChainNoise.getValues(reducedNoiseSamples, position - size / 2f, size);
            //float[,] noiseMountainChainData = new float[texture.Width, texture.Height];

            float[,] waterChanceData = land.waterChanceNoise.getValues(noiseSamples, position - size / 2f, size); ;
            ...
            for (int i = 0; i < texture.Width * texture.Height; i++)
            {
                ...
                x = (i % texture.Width);
                y = (i / texture.Width);
                reducedx = x/noiseDownSampling;
                reducedy = y/noiseDownSampling;

                grassRoughness = (((float)rdm.NextDouble() - 1f) * 0.3f) * (landGreenessNoiseData[reducedx, reducedy]);
                grassColor = Color.Lerp(Color.Lerp(Color.Black, Color.DarkGreen, (float)Math.Sqrt(landGreenessNoiseData[reducedx, reducedy])), Color.LawnGreen, landGreenessNoiseData[reducedx, reducedy] + grassRoughness - noiseDirtData[reducedx, reducedy] * 0.3f);

                dirtRoughness = (((float)rdm.NextDouble() - 1f) * 0.3f);
                dirtColor = Color.Lerp(Color.Black, Color.Lerp(Color.SaddleBrown, Color.Black, 0.1f), noiseDirtData[reducedx, reducedy] + dirtRoughness + 0.1f);

                noise = landGreenessNoiseData[reducedx, reducedy] * 0.7f + noiseDirtData[reducedx, reducedy] * 0.3f;
                ...
                if (noiseMountainChanceData[reducedx, reducedy] < land.mountainChanceUpperValue*1.2f && noiseMountainChainData[reducedx, reducedy] < land.mountainChainUpperValue*1.2f)
                {
                    mountainDist = (land.mountainChanceUpperValue*1.2f - noiseMountainChanceData[reducedx, reducedy]) / (land.mountainChanceUpperValue*1.2f);
                    if (mountainDist > (land.mountainChainUpperValue*1.2f - noiseMountainChainData[reducedx, reducedy]) / (land.mountainChainUpperValue*1.2f))
                        mountainDist = (land.mountainChainUpperValue*1.2f - noiseMountainChainData[reducedx, reducedy]) / (land.mountainChainUpperValue*1.2f);
